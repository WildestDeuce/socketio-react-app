...
module.exports = function (client, clientManager, chatroomManager) {
    ...
    function handleRegister(userName, callback) {
        if (!clientManager.isUserAvailable(userName))
            return callback("usr is not available")
    ...
    function handleJoin(chatroomName, callback) {
        ...
        handleEvent(chatroomName, createEntry)
            .then(function (chatroom) {
                ...
                //send chat histroy to client
                callback(null, chatroom.getChatHistory())
            })
            .catch(callback)
    }

    function handleLeave(chatrooms, callback) {
    ...
    function handleGetAvailableUsers(_, callback) {
        return callback(null, clientManager.getAvaialbleUsers())
    }
